...
void tt() {
    cout << "starting module testing." << endl;
      ...
      readCsvFileAsLandmarkList("test/data/test_data_full.csv", 1000000);
      testCapturedStandPose(fitInstance, true);
      // //round one 
      testFittingStandPose(fitInstance);

    //   // //scale up and down full data error 
      readCsvFileAsLandmarkList("test/data/test_data_heavy.csv", 1000000);
      ...
      readCsvFileAsLandmarkList("test/data/test_data_full.csv", 1000000);
      testFittingBoneLengthBiasCalculationErrorFullSequence(fitInstance);
    readCsvFileAsLandmarkList("test/data/rgbd_stand_pose_test.csv", 1000000);
